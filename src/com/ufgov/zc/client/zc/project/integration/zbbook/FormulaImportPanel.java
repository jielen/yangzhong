...
import com.ufgov.zc.client.zc.ztb.service.FormulaImportService;
...
public class FormulaImportPanel extends JPanel {

  private static final long serialVersionUID = -3673893045375937678L;

  private FormulaImportService formulaImportService;
  ...
  public FormulaImportPanel(MainPanel mainPanel, JDialog dialog) throws Exception {
    this.mainPanel = mainPanel;
    this.dialog = dialog;
    formulaImportService = new FormulaImportService();
    initPanel();
  }

  public void initPanel() throws Exception {
    this.setLayout(new BorderLayout());
    ZTBButton impBtn = new ZTBButton("dbFormula");
    ...
    impBtn.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        try {
          ...
          formulaImportService.updateBagDetails(checkedCodes, filePath, mainPanel);
  ...
  private List<Map<String, String>> buildTableRowData() throws Exception {
    ...
    if (GV.NODE_TYPE_DOC.equals(nodeType)) {
      ...
      if (projNode.getNodeCode() == null) {
        String tipMeg = GV.getOperateMsg("dbImportRequirement.unsupportedDocNode", null);
        throw new Exception(tipMeg);
      } else {
        projNo = projNode.getNodeCode();
      }
    }
    if (projNo == null) {
      String tipMeg = GV.getOperateMsg("dbImportFormula.unsupportedDocNode", null);
      throw new Exception(tipMeg);
    }
    return formulaImportService.getFormulaFilesList(projNo);
  }
