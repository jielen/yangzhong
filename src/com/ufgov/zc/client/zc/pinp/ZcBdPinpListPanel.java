...
import com.ufgov.zc.client.common.converter.zc.ZcBdPinpToTableModelConverter;
...
/**
* @ClassName: ZcBdPinpListPanel
* @Description: 协议采购商品品牌
* @date: 2012-03-21
* @version: V1.0 
* @since: 1.0
* @author: shenwanchao
* @modify: 
*/
public class ZcBdPinpListPanel extends AbstractEditListBill implements ParentWindowAware {
  private static final Logger logger = Logger.getLogger(ZcBdPinpListPanel.class);

  public static final String compoId = "ZC_ZB_PINP";
  ...
  public IZcBdPinpServiceDelegate zcBdPinpServiceDelegate = (IZcBdPinpServiceDelegate) ServiceFactory.create(IZcBdPinpServiceDelegate.class,
    "zcBdPinpServiceDelegate");
  ...
  public IZcBdPinpServiceDelegate getZcBdPinpServiceDelegate() {
    return zcBdPinpServiceDelegate;
  }

  public void setZcBdPinpServiceDelegate(IZcBdPinpServiceDelegate zcBdPinpServiceDelegate) {
    this.zcBdPinpServiceDelegate = zcBdPinpServiceDelegate;
  }
  ...
  private final class DataDisplay extends MultiDataDisplay {
    private DataDisplay(List<TableDisplay> displays, List<TableDisplay> showingDisplays, AbstractSearchConditionArea conditionArea,
      ...
      super(displays, showingDisplays, conditionArea, showConditionArea, ZcSettingConstants.TAB_ID_ZC_ZB_PINP);
      setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), LangTransMeta.translate("ZC_ZB_PINP_TITLE"),
        ...
        table.addMouseListener(new MouseAdapter() {
          ...
          public void mouseClicked(MouseEvent e) {
            if (e.getClickCount() == 2 && SwingUtilities.isLeftMouseButton(e)) {
              ...
              getCheckedList();
              new ZcBdPinpDialog(ZcBdPinpListPanel.this, viewList, row, tabStatus);
            }
          }
        });
      }
    ...
    protected void handleTableDisplayActived(AbstractSearchConditionItem[] searchConditionItems, final TableDisplay tableDisplay) {
      ...
      UIUtilities.asyncInvoke(new DefaultInvokeHandler<TableModel>() {
        ...
        public TableModel execute() throws Exception {
          return ZcBdPinpToTableModelConverter.convertToTableModel(ZcBdPinpListPanel.this.zcBdPinpServiceDelegate.getPinpList(elementConditionDto,
            requestMeta));
        }
    ...
    UIUtilities.asyncInvoke(new DefaultInvokeHandler<List<SearchCondition>>() {
      ...
      public List<SearchCondition> execute() throws Exception {
        List<SearchCondition> needDisplaySearchConditonList = SearchConditionUtil.getNeedDisplaySearchConditonList(WorkEnv.getInstance()
          .getCurrUserId(), ZcSettingConstants.TAB_ID_ZC_ZB_PINP);
  ...
  private AbstractSearchConditionArea createTopConditionArea() {
    ...
    topSearchConditionArea = new SaveableSearchConditionArea(ZcSettingConstants.CON_ID_ZC_ZB_PINP, null, true, defaultValueMap, null);
  ...
  private AbstractDataDisplay createDataDisplay(List<TableDisplay> showingDisplays) {
    return new DataDisplay(SearchConditionUtil.getAllTableDisplay(ZcSettingConstants.TAB_ID_ZC_ZB_PINP), showingDisplays, createTopConditionArea(),
  ...
  public void refreshCurrentTabData(List beanList) {
    topDataDisplay.getActiveTableDisplay().getTable().setModel(ZcBdPinpToTableModelConverter.convertToTableModel(beanList));
  }
   ...
   */
  public void doAdd() {
    new ZcBdPinpDialog(ZcBdPinpListPanel.this, new ArrayList(), this.topDataDisplay.getActiveTableDisplay().getTable().getRowCount(), topDataDisplay
  ...
  private void doDelete() {
    ...
    try {
      for (int i = 0; i < checkedList.size(); i++) {
        ZcBBrand pinp = (ZcBBrand) checkedList.get(i);
        zcBdPinpServiceDelegate.deleteByPrimaryKey(pinp.getZcBraCode(), requestMeta);
      }
...
//      return;
//    }
    int num = JOptionPane.showConfirmDialog(this, "确认作废本次品牌?", "审核", 0);
    ...
    try {
      for (int i = 0; i < beanList.size(); i++) {
        ZcBBrand zcBdPinp = (ZcBBrand) beanList.get(i);
        zcBdPinp.setZcZbPpStatus("cancel");
        zcBdPinp.setZcOperCode(requestMeta.getSvUserID());
        //zcBdPinp.setComment(commentDialog.getComment());
        this.zcBdPinpServiceDelegate.updateByPrimaryKey(zcBdPinp, requestMeta);
      }
    } catch (Exception e) {
      success = false;
    ...
    if (success) {
      JOptionPane.showMessageDialog(this, "作废品牌成功！", "提示", JOptionPane.INFORMATION_MESSAGE);
      this.refreshCurrentTabData();
    } else {
      JOptionPane.showMessageDialog(this, "作废品牌失败 ！" + errorInfo, "错误", JOptionPane.ERROR_MESSAGE);
    }
  }
...
//    }
    ...
    if (success) {
      JOptionPane.showMessageDialog(this, "暂停品牌成功！", "提示", JOptionPane.INFORMATION_MESSAGE);
      this.refreshCurrentTabData();
    } else {
      JOptionPane.showMessageDialog(this, "暂停品牌失败 ！" + errorInfo, "错误", JOptionPane.ERROR_MESSAGE);
    }
  }
...
//    }
    ...
    if (success) {
      JOptionPane.showMessageDialog(this, "启用商品品牌成功！", "提示", JOptionPane.INFORMATION_MESSAGE);
      this.refreshCurrentTabData();
    } else {
      JOptionPane.showMessageDialog(this, "启用商品品牌失败 ！" + errorInfo, "错误", JOptionPane.ERROR_MESSAGE);
    }
  }
  ...
  private void doPrint() {
    List<ZcBBrand> printList = getCheckedList();
    if (printList.size() == 0) {
  ...
  private void doPrintPreview() {
    final List<ZcBBrand> printList = getCheckedList();
    if (printList.size() == 0) {
  ...
  public static void main(String[] args) {
    SwingUtilities.invokeLater(new Runnable() {
      public void run() {
        ...
        ZcBdPinpListPanel bill = new ZcBdPinpListPanel();
