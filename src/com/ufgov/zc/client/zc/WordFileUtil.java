...
public class WordFileUtil {
  ...
  private static IBaseDataServiceDelegate baseDataServiceDelegate = (IBaseDataServiceDelegate) ServiceFactory.create(IBaseDataServiceDelegate.class, "baseDataServiceDelegate");
  ...
  public static String createFile(String fileName, byte[] content) throws Exception {
    ...
    try {
      ...
      fullFileName = "";
      JOptionPane.showMessageDialog(null, "从服务器载入文件 内容失败,检查服务器上此文件是否存在！", "错误", JOptionPane.ERROR_MESSAGE);
    } finally {
      if (fos != null) {
        try {
          fos.close();
