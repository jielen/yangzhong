...
public class ProduceConsume {
  ...
  public static void main(String[] args) {
    ...
    ProducerMantou p = new ProducerMantou(ss);//新建一个生产者，使之持有框  
    ConsumeMantou c = new ConsumeMantou(ss);//新建一个消费者，使之持有同一个框  
...
class SyncStack {
  ...
  public synchronized SteamBread pop() {
    ...
    System.out.println("消费了" + stb[index]);
    return stb[index];
  }
}

//生产者类，实现了Runnable接口，以便于构造生产者线程  
class ProducerMantou implements Runnable {
  SyncStack ss = null;

  ProducerMantou(SyncStack ss) {
    this.ss = ss;
  }
...
class ConsumeMantou implements Runnable {
  SyncStack ss = null;

  public ConsumeMantou(SyncStack ss) {
    super();
    this.ss = ss;
  }
