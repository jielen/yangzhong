...
import com.ufgov.zc.client.zc.supplierprice.model.DdcgPriceDetailPackTableModel;
...
public class DdcgPriceDetailEditPanel extends JPanel implements ActionListener {
  ...
  private JTabbedPane packDetailPane = new JTabbedPane();

  private JTablePanel signupPackDetail = new JTablePanel("signupPackDetail");//分项报价表表格
  ...
  List<ZcEbSignupPackDetail> details;
  ...
  public void setSignup(ZcEbSignup signUp) {
    ...
    details = zcEbSignupServiceDelegate.getZcEbSignupPackDetail(dto, meta);

    this.signup.setSignupPacks(details);

    signupPackDetail.init();
    signupPackDetail.getSearchBar().setVisible(false);
    signupPackDetail.setTablePreferencesKey(this.getClass().getName() + "_detail_table");
    packDetailPane.addTab("上传报价明细表", signupPackDetail);

    this.signupPackDetail.setTableModel(DdcgPriceDetailPackTableModel.convertPackToTableModel(details));

    ZcUtil.translateColName(signupPackDetail.getTable(), DdcgPriceDetailPackTableModel.getPackTableColumnInfo());
    JTable table = signupPackDetail.getTable();

    table.setDefaultEditor(String.class, new TextCellEditor());
    SwingUtil.setTableCellEditor(table, "TBYLB_FILE_NAME", new FileCellEditor("tbylbFileName", "tbylbFileId", new String[] {"doc", "docx" , "xls", "xlsx"}, false));
    ...
    ((BeanTableModel) signupPackDetail.getTable().getModel()).setEditable(false);
  ...
  private void doSave() {
    try {
      ...
      for (ZcEbSignupPackDetail detail : details) {
        if (detail.getTbylbFileId() == null) {
          sb.append(detail.getPackDesc() + "没有上传报价明细表\n");
        }
      }
      ...
      zcEbBaseServiceDelegate.updateObjectList("ZcEbSignup.updateZcEbSignupPackDetailTbYLBID", details, meta);
  ...
  private void doUpdate() {
    ((BeanTableModel) signupPackDetail.getTable().getModel()).setEditable(true);
