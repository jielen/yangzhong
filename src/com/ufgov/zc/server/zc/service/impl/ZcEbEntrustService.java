...
public class ZcEbEntrustService implements IZcEbEntrustService {
  ...
  private List getOriginZcEbEntrust(List currBeanList) {
    ...
    for (int i = 0; i < currBeanList.size(); i++) {
      beanList.add(this.getZcEbEntrustBySn(((ZcEbEntrust) currBeanList.get(i)).getSn()));
    }
    return beanList;
  }
  ...
  private void checkConsistency(List currBeanList) {
    ZcSUtil.checkConsistency(currBeanList, getOriginZcEbEntrust(currBeanList), "sn");
  }
  ...
  private Map getZcEbEntrustDetailListMap(List zcEbEntrustList) {
    ...
    for (int i = 0; i < subDataList.size(); i++) {
      CpPlanAgentList cpPlanAgentList = (CpPlanAgentList) subDataList.get(i);
      List oldList = (List) dataMap.get(cpPlanAgentList.getPlanAgentBillId());
      if (oldList != null) {
  ...
  public void doBack(ZcEbEntrust entrust, RequestMeta requestMeta) {
    checkConsistency(entrust);
    zcEbEntrustDao.updateZcEbEntrustStatus(entrust);
    if (entrust.getProcessInstId() != null && entrust.getProcessInstId().longValue() > 0) {
      // 如果流程已经启动，先终止流程
      wfEngineAdapter.interrupt(entrust.getComment(), entrust, requestMeta);
    }
    ZcPProMake zcPProMake = zcPProMakeDao.selectByPrimaryKey(entrust.getZcMakeCode());
    // 采购计划工作流销审
    WorkflowContext workflowContext = wfEngineAdapter.genCommonWFC(entrust.getComment(), zcPProMake, "ZC_P_PRO_MAKE", requestMeta);
  ...
  public ZcEbEntrust doSaveFN(ZcEbEntrust entrust, RequestMeta requestMeta) {
    ...
    if (entrust.getSn() == null || entrust.getSn().equals("自动编号") || entrust.getSn().equals("")) {
      ...
      //      System.out.println("service.doSaveFN 2" + entrust.getCoCode());

    } else {

      //      System.out.println("service.updateZcEbEntrust 1" + entrust.getCoCode());
      zcEbEntrustDao.updateZcEbEntrust(entrust);
  ...
  public int deleteByPrimaryKey(ZcEbEntrust zcEbEntrust) {
    if (zcEbEntrust != null && zcEbEntrust.getSn() != null) {
      ZcPProMake zcPProMake = this.zcPProMakeDao.selectByPrimaryKey(zcEbEntrust.getZcMakeCode());

      if (zcPProMake != null) {
        // 删除采购计划的信息
        zcPProMakeDao.deleteByPrimaryKey(zcPProMake.getZcMakeCode());
        // 删除资金明细
        baseDao.delete("ZC_P_PRO_MITEM_BI.deleteByZcMakeCode", zcPProMake.getZcMakeCode());
        // 删除采购明细
        baseDao.delete("ZC_P_PRO_MITEM.deleteMitemByZcMakeCode", zcPProMake.getZcMakeCode());
      }
      // 删除任务单明细
      baseDao.delete("ZcEbEntrust.deleteZcEbEntrustDetaiBi", zcEbEntrust.getZcMakeCode());
  ...
  public List getTodoListByUser(List compId, String userId) {
    ...
    for (int i = 0; i < compId.size(); i++) {
      String table = (String) baseDao.read("consoleChart.getCompoTableById", compId.get(i));
      if (table == null) {
        continue;
      }
      para.put("table", table);
      para.put("compo_id", compId.get(i));
      String compoid = (String) baseDao.read("consoleChart.getCompoCurrentTaskByCompoAndUserId", para);
