...
public class HuiyuanUserService implements IHuiyuanUserService {
  ...
  public HuiyuanUser saveFN(HuiyuanUser record, RequestMeta requestMeta) throws BusinessException {
    ...
    if (record.getUserguid() == null || record.getUserguid().trim().length() == 0) {
      ...
      if (existCa(record, requestMeta, isNewUser, null)) { throw new BusinessException("CA证书号" + record.getDognum() + "已经被使用!"); }
      //2.loginId是否已经被使用
      if (existLoginId(record, requestMeta, isNewUser, null)) { throw new BusinessException("登陆账号名" + record.getLoginid() + "已经被使用!"); }
      ...
      //1.ca非空时，ca是否被用,根据新的ca从表huiyuan_user和as_emp获取数据进行判断
      if (existCa(record, requestMeta, isNewUser, existUser.getLoginid())) { throw new BusinessException("CA证书号" + record.getDognum() + "已经被使用!"); }
      //2.loginId是否已经被使用
      if (existLoginId(record, requestMeta, isNewUser, existUser.getLoginid())) { throw new BusinessException("登陆账号名" + record.getLoginid() + "已经被使用!"); }
  ...
  private boolean existLoginId(HuiyuanUser record, RequestMeta requestMeta, boolean isNewUser, String oldLoginId) {
    // TCJLODO Auto-generated method stub
    if (record == null || record.getLoginid() == null || record.getLoginid().trim().length() == 0) { return false; }
    ...
    for (int i = 0; i < userLst.size(); i++) {
      ...
      if (!existUser.getUserguid().equals(record.getUserguid())) { return true; }
    }
    ...
    if (user == null) {
      ...
      if (isNewUser) {//新增用户
        ...
        if (!user.getUserId().equals(oldLoginId)) { return true; }
      }
    }
    return false;
  }
  ...
  private boolean existCa(HuiyuanUser record, RequestMeta requestMeta, boolean isNewUser, String oldLoginId) {
    // TCJLODO Auto-generated method stub
    if (record == null || record.getDognum() == null || record.getDognum().trim().length() == 0) { return false; }
    ...
    for (int i = 0; i < userLst.size(); i++) {
      HuiyuanUser existUser = (HuiyuanUser) userLst.get(i);
      if (!existUser.getUserguid().equals(record.getUserguid()) && existUser.getDognum().equals(record.getDognum())) { return true; }
    }
