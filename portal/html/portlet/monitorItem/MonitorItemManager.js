 ...
 */
var currentNode;
ufgov.portal.portlet.MonitorItemManager = function(){

};
Ext.extend(ufgov.portal.portlet.MonitorItemManager, ufgov.portal.Base, {
    pageInit: function(){
        this.createMainFrame();
    },
    createMainFrame: function(){
        var receiveFileBox = this.getFileClass('receiveFile', '文件类别', '02', 'pThis.getFileBoxData', userId);
        new Ext.Viewport({
            layout: 'border',
    ...
    getFileBoxData: function(node, userId){
        MonitorItemManager.getFileBoxGridData(node, userId);
    },
    getFileBoxGridData: function(node, userId){
                    ...
                    Ext.Msg.confirm('系統提示', '确认删除吗?(注意,该删除不可恢复！)', function(btn){
                        ...
                        Ext.Ajax.request({
                            ...
                            callback: function(option, success, response){
                                if (success) {
                                    var result = Ext.util.JSON.decode(response.responseText);
                                    MonitorItemManager.showMsg('温馨提示', result.sign);
                                    fileBoxStore.reload();
                                }
                            }
                        });
         ...
         * 销毁原面板
         */
        MonitorItemManager.destroyPanel(MonitorItemManager.fileBoxDetailPanel);
        /**
         * 创建新面板
         */
        MonitorItemManager.fileBoxDetailPanel = new Ext.Panel({
            border: false,
            layout: 'fit',
        ...
        //		Ext.getCmp('winMore').on('resize', function(){
        //			fileBoxPanel.setWidth(Ext.getCmp('fileMainPanel').getInnerWidth());
        //			fileBoxPanel.syncSize();
        //		});
        Ext.getCmp('fileMainPanel').on('resize', function(){
            MonitorItemManager.fileBoxDetailPanel.setWidth(Ext.getCmp('fileMainPanel').getInnerWidth());
            MonitorItemManager.fileBoxDetailPanel.syncSize();
        });
    },
...
var MonitorItemManager = new ufgov.portal.portlet.MonitorItemManager();

Ext.onReady(function(){
    Ext.QuickTips.init();
    MonitorItemManager.pageInit();
});
