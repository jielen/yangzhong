...
谈判结果报告</span><span lang="EN-US" style="font-size: 36.0pt; mso-bidi-font-size: 12.0pt; mso-fareast-font-family: 方正姚体; color: black"><o:p></o:p></span></b></p>
...
），由西安市市级单位政府采购中心采用竞争性谈判的方式组织实施，于<%=projReportDto.getOpenBidDate()%>在<%=projReportDto.getOpenBidAddress()%>进行。
<%=projReportDto.getBidReportPublishDate()%>在陕西省政府采购网发出公告后，有<%=projReportDto.getSingupSupplierCount()%>家供应商前来登记并购买谈判文件，其中
<%
for(int i = 0;i < projReportDto.getPackList().size();i++){
   PackReportDto pack = (PackReportDto)(projReportDto.getPackList().get(i));
...
%>
。谈判截止时间前实际收到<%=projReportDto.getSubmitFileSupplierCount()%>家供应商的谈判响应文件，其中：
<%
for(int i = 0;i < projReportDto.getPackList().size();i++){
   PackReportDto pack = (PackReportDto)(projReportDto.getPackList().get(i));
   out.print(pack.getPackName());
...
%>。（<b>如果谈判截止时间结束后参加谈判的供应商不足三家的，除采购任务取消情形外，应注明谈判文件没有不合理条款、谈判公告时间及程序符合规定，现场征求采购人意见同时报政府采购管理部门同意，继续进行或者就地转为询价或单一来源方式采购。如果谈判文件存在不合理条款、谈判公告时间及程序不符合规定，应予废标，并依法重新招标。如在谈判期间，出现符合专业条件的供应商或者对谈判文件作出实质响应的供应商不足三家情形的，参照上述执行，政府采购法第四十三条</b>）谈判小组按照谈判文件的要求，本着公开、公平、公正的原则，经过资质审查、综合评审打分，推荐了成交候选单位。具体情况如下：<br />
一、公开报价情况<br />
</p>
<%
String[] bjSeri = { "一", "二", "三", "四", "五", "六", "七", "八", "九", "十" };
for(int i = 0;i < projReportDto.getPackList().size();i++){
    PackReportDto pack = (PackReportDto)(projReportDto.getPackList().get(i));
%>
...
<table id="chTable" cellpadding="0" cellspacing="0" border="1" align="center" style="border-color:black;width: 600px;border-collapse:collapse;">
	<tr>
		<td>谈判供应商</td>
		<%
		    for(int c = 0;c < pack.getMaxBjCount().intValue() && c < bjSeri.length;c++){
		%>
		       <td>第<%=bjSeri[c]%>次总报价（元）</td>
		<%
		    }
		%>
		<td>交货期</td>
...
%>	
	<tr>
		...
		<%
		    List prices = supplier.getPrices();
		    for(int f = 0;f < prices.size();f++){
		        PriceInforDto price = (PriceInforDto )prices.get(f);
		%>
		<td><%=price.getPrice()%>&nbsp;</td>
		<%
		    }
		    for(int c = 0;c < pack.getMaxBjCount().intValue() - prices.size();c++){
		%>
		<td>&nbsp;</td>
		<%
		    }
...
%>
<p>二、资质审查情况</p>
<p>根据谈判文件的资格审核要求，谈判小组成员对<%=projReportDto.getPackCount()%>个标段<%=projReportDto.getSubmitFileSupplierCount()%>家谈判响应供应商的谈判资格进行了审核，均符合谈判文件的资格要求。（如有资格不符合谈判文件要求的准确说明原因）</p>
<p>三、综合评审打分情况</p>
<p>根据谈判文件的综合评审要求, 谈判小组成员对各家供应商的谈判响应文件进行了综合评审，并进行了打分、排序。</p>
<p style="text-align:center">谈判小组评审总得分</p>
<%
for(int i = 0;i < projReportDto.getPackList().size();i++){
    PackReportDto pack = (PackReportDto)(projReportDto.getPackList().get(i));
%>
...
%>
	<tr>
		...
		<td><%=getNotNullVal(supplier.getAverageScore())%>&nbsp;</td>
	</tr>
<%
    }
%>
...
%>
<p>四、谈判结论</p>
<p>综合以上情况，谈判小组成员依照谈判文件和评审办法的要求，综合打分后排序如下：</p>
<%
for(int i = 0;i < projReportDto.getPackList().size();i++){
    PackReportDto pack = (PackReportDto)(projReportDto.getPackList().get(i));
%>
...
<p align="right">西安市<%=projReportDto.getProjName()%>项目谈判小组</p>
