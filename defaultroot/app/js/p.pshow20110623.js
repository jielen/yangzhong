...
/*getCommentSummary*/
(function(){var skuid=$("#name").attr("PshowSkuid"),getCommentUrl="http://club.360buy.com/ProductPageService.aspx?method=GetCommentSummaryBySkuId&referenceId={0}&callback=?".format(skuid);$.getJSON(getCommentUrl,function(result){if(!result)return;var domobj=$("#star"+result.SkuId);if(!domobj.html()){domobj=$("div[id^='star']")}domobj.html("<div class=\"star sa"+result.AverageScore+"\"></div><a href=\"http://www.360buy.com/product/"+result.SkuId+".html#comment\" class=\"num-comment\">(已有"+result.CommentCount+"人评价)</a>")})})();
/*fitting*/
...
/*gift*/
var GiftCardWid=null;function getGift(wid){$.jdThickBox({type:"json",width:600,height:530,title:"选择贺卡样式",source:"http://www.360buy.com/giftAjax.aspx?getGift=get&wid="+wid+"&jsoncallback=?"},function(url,ele){$.getJSON(url,function(json){if(json.info!=""){ele.html(json.info)}})})}function setGift(){var array=document.getElementById("changeCard").getElementsByTagName("input");var wid;for(var i=0;i<array.length;i++){if(array[i].type=='radio'&&array[i].checked){wid=array[i].value;break}}var to=document.getElementById("To").value;var form=document.getElementById("From").value;var msg=document.getElementById("Message").value;to=to.replace(/<[\s\S]*?>/,"");form=form.replace(/<[\s\S]*?>/,"");msg=msg.replace(/<[\s\S]*?>/,"");GiftCardWid=wid;if(wid==""||wid==null){alert("请选择贺卡样式！");return false}if(to==""||to==null){alert("请填写贺卡收件人！");return false}if(msg==""||msg==null){alert("请填写贺卡内容！");return false}if(msg.length>80){alert("贺卡内容长度不能超过80！目前已经输入"+msg.length);return false}if(form==""||form==null){alert("请填写贺卡发件人！");return false}$.getJSON("http://www.360buy.com/giftAjax.aspx?getGift=set&wid="+wid+"&To="+to+"&From="+form+"&msg="+msg+"&jsoncallback=?",function(json){if(json.info!=""){if(json.info=="true"){if(CardUrl!=null){window.location.href=CardUrl}else if(CardFun!=null){eval(CardFun)}}else{alert("保存信息失败！")}}})}var CardUrl=null;var CardFun=null;function Card(){this.Wid=0;this.Url='';this.Fun='';this.Show=function(){CardUrl=this.Url;CardFun=this.Fun;getGift(this.Wid)}}
...
function yb_showYb(skuId){
	...
	setTimeout(function(){document.getElementsByTagName('head')[0].appendChild(js);},2000);
}
function viewFqMarketingInfo(){
	var source="http://www.360buy.com/template/creditcard.htm?wid="+fq_skuId;
	setTimeout(function(){
		$.jdThickBox({
			type:"iframe",
			title:"大家电领航，招行分期优惠",
...
function fqMarketingService(result){
	if (result&&result.flag){
		$("#storeinfo").css({"z-index":2});
		$("#fqPanel").css({"position":"relative","z-index":1}).append("<div style='position:absolute;top:35px;left:-70px;width:225px;height:20px;padding:17px 10px 6px 5px;background:url(http://misc.360buyimg.com/201007/skin/df/i/bg_fqmarketing.gif) no-repeat 0 0;'><b style='float:right;margin-top:5px;width:7px;height:7px;background:url(http://misc.360buyimg.com/201007/skin/df/i/bg_fqmarketing.gif) no-repeat 0 -45px;' onclick=\"this.parentNode.style.display='none'\"></b>招商银行分期付款优惠啦！<a href='javascript:void(0)' onclick='viewFqMarketingInfo()'>了解详情&gt;&gt;</a></div>");
	}
}
...
/*adservice*/
var AdServiceUrl="http://www.360buy.com/ajaxService.aspx";function GetAdWordCallback(json){if(json){$("#advertiseWord").html(json.html)}};function GetSingleAdWord(skuId){asyncScript(AdServiceUrl+"?callback=GetAdWordCallback&stype=single&skuid="+skuId,GetAdWordCallback)};
/*comment*/
...
/*landingpage*/
function showlangdingpage(){var landingPage={source:["http://www.baidu.com","http://www.google.com","http://www.google.com.hk","http://www.google.com.hk","http://click.union.360buy.com"],checkSource:function(url){if(!url)return true;var i=0,j=this.source.length;for(i=0;i<j;i++){if(url.toLowerCase().indexOf(this.source[i].toLowerCase())==0)return true}return false},isShow:function(skuid){if(this.checkSource(document.referrer.toLowerCase())&&!getCookie("pin")&&typeof stockdata!="undefined"&&skuid)return true;return false},stock:function(){if(typeof stockdata=="undefined")return false;var i=0,j=stockdata.length;for(i=0;i<j;i++){if(stockdata[i].Stock==33||stockdata[i].Stock==36||stockdata[i].Stock==39)return true}return false},checkEmail:function(v){var reg=/^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+((\.[a-zA-Z0-9_-]{2,3}){1,2})$/;return reg.test(v)}};function checkEmail(mail){var obj=$("#emailalert span");obj.html("");if(!mail){obj.html("Email不能为空！");return false}if(!landingPage.checkEmail(mail)){obj.html("Email格式错误！");return false}return true}var skuid=$("#name").attr("pshowskuid");if(landingPage.isShow(skuid)){var html="<div id='toolbar' style='display:none'><span class='line clr'></span><div class='w'><div class='slogan'><div class='i-slogan'><a class='fore1' target='_blank' href='http://help.360buy.com/help/question-55.html' clstag='shangpin|keycount|landingpage|jialianwumei' title='正品行货又便宜'>正品保障</a><span>|</span><a class='fore2' target='_blank' href='http://help.360buy.com/help/question-65.html' clstag='shangpin|keycount|landingpage|211' title='上午下单当日送达'>211限时达</a><span>|</span><a class='fore3' target='_blank' href='http://help.360buy.com/help/question-65.html' clstag='shangpin|keycount|landingpage|mianyunfei' title='不限金额地区'>全场免运费</a><span>|</span><a class='fore4' target='_blank' href='http://help.360buy.com/help/question-97.html' clstag='shangpin|keycount|landingpage|100fen' title='收检100分钟内给出处理意见'>售后100分</a></div></div><div id='recommend-samekind' class='samekind'><div class='entrance'><a href='#' clstag='shangpin|keycount|landingpage|more'>查看更多同类商品</a><b></b></div></div><div class='form'><div class='prompt' id='emailalert'><div class='i-prompt'><span></span><div class='corner corner-bottom'><b class='b1'></b><b class='b2'></b></div></div></div><div class='label'><b></b>想率先获知本商品<strong>促销降价</strong>吗？</div><input type='text' value='请留下您的Email' class='text'/><input type='button' value='订阅降价通知' class='button' clstag='shangpin|keycount|landingpage|jiangjia'/></div><span class='clr'></span></div></div>";$(html).insertAfter("#footer");var moreurl=$($(".crumb a")[3]).attr("href");if(!moreurl)moreurl="http://www.360buy.com";$("#recommend-samekind a").attr("href",moreurl);var stock=landingPage.stock();var type=stock?1:2;var key=$.trim($("#skuidkey").html());if(!stock){$("#toolbar .label").html("<b class='markdown'></b>一旦到货，我们会立即通知您！");$("#toolbar .button").val("订阅到货通知")}$("#toolbar .text").focus(function(){if($.trim($(this).val())=="请留下您的Email")$(this).val("")}).blur(function(){checkEmail($.trim($(this).val()))});$("#toolbar .button").click(function(){var mail=$.trim($("#toolbar .text").val());if(mail=="请留下您的Email")return;var obj=$("#emailalert span");if(checkEmail(mail)){$("#emailalert").attr("class","prompt")}else{$("#emailalert").attr("class","prompt active");return}var that=$(this);that.attr("disabled","disabled");$.ajax({url:"http://jd2008.360buy.com/NotifyAnonymous.ashx",data:{action:"submitnotify",skuid:skuid,type:type,key:key,mail:mail},dataType:"jsonp",success:function(r){$("#emailalert span").html(r.message);$("#emailalert").attr("class","prompt active");that.attr("disabled","");if(r.result){setTimeout("$('#emailalert').attr('class','prompt')",2000)}}})});$("#toolbar").show();try{log("dtoolbar","show");}catch(e){}}}
/*serviceAgent*/
(function(){var pid=$("#name").attr("pshowskuid"),cid=$("#name").attr("serviceAgentId").split(",")[3];if(cid=="471"||cid=="818"||cid=="52"||cid=="508"){asyncScript("http://chat.360buy.com/api/check.action?pid="+pid);}else{showlangdingpage();}})();
/*
